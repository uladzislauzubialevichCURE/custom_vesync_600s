...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
   ...
   for vs_p in PLATFORM_TO_CONST.values():
       hass.data[DOMAIN][config_entry.entry_id][vs_p] = device_dict.get(vs_p, [])

   # Set up platforms with new setup method
   ...
   async def async_new_device_discovery(service: ServiceCall) -> None:
       ...
       def _add_new_devices(platform: Platform) -> None:
           ...
           vs_platform = PLATFORM_TO_CONST[platform]
           old_devices = hass.data[DOMAIN][config_entry.entry_id][vs_platform]
           if new_devices := list(
               set(dev_dict.get(vs_platform, [])).difference(old_devices)
           ):
               old_devices.extend(new_devices)
               async_dispatcher_send(
                   hass, VS_DISCOVERY.format(vs_platform), new_devices
               )
